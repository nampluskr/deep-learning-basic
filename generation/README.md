...
class AuxCondGAN(GAN):
    ...
    def train_step(self, data):
        r_images, labels = data
        labels_argmax = labels.argmax(-1) if labels.shape[1] > 1 else labels ## One-hot
        ...
        d_loss = r_loss + f_loss
        d_loss += self.loss_fn_aux(r_preds, labels_argmax)
        d_loss += self.loss_fn_aux(f_preds, labels_argmax)
        ...
        g_loss = self.loss_fn(g_logits, torch.ones_like(g_logits))
        g_loss += self.loss_fn_aux(g_preds, labels_argmax)
...
def train(gan, train_loader, args, sample_noises):
    ...
    for epoch in range(args.n_epochs):
        ...
        with tqdm(train_loader, total=len(train_loader), ncols=100,
            ...
            for x, y in pbar:
                data = x.to(gan.device), y.to(gan.device)
        ...
        if (epoch + 1) % args.log_interval == 0:
            sample_images = gan.G(sample_noises).cpu().detach().numpy()
            sample_images = sample_images.reshape(-1, 28, 28)*255
            img_name = os.path.join(log_path, args.log_dir + "-%04d.png" % (epoch+1))
            save_images(sample_images, img_name=img_name)

    torch.save(gan.G.state_dict(), os.path.join(log_path, args.log_dir + "_weights.pth"))
    return hist
```
